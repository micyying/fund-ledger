...
目前版本：**GitHub版 · v1.5.0**（2026-09-26）。資產總覽新增「我的配置計畫」：可設定四類基金的目標區間、逐檔分類，以及恆指／中國新經濟持倉的回升觀察淨值。當類別超出目標上限，而且已儲存淨值達到自訂觀察價時，會在開啟帳本後顯示檢視提醒；不會自動交易。不同幣種的市值需手動輸入換算匯率，缺資料時不顯示不完整的配置比例。上一版的持倉配置圖、資金進出圖及版本改動說明亦包含在這次更新中。
...
| 資料表（基金持倉、銀行／平台彙總、每月資金進出） | ✅ 正常；現金流與投資報酬分開呈現 |
